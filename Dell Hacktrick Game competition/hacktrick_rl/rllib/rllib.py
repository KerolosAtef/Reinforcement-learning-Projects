...
from hacktrick_ai_py.mdp.hacktrick_mdp import HacktrickGridworld, EVENT_TYPES
from hacktrick_ai_py.agents.benchmarking import AgentEvaluator
from hacktrick_ai_py.agents.agent import Agent, AgentPair, StayAgent
...
from hacktrick_rl.rllib.utils import softmax, get_base_ae, get_required_arguments, iterable_equal
...
class HacktrickMultiAgent(MultiAgentEnv):
    """
    ...
    def __init__(self, base_env, reward_shaping_factor=0.0, reward_shaping_horizon=0,
                            bc_schedule=None, use_phi=True, mode='single'):
        """
    ...
    # Hacktrick Participant Critical Message: ################################
    def get_dense_reward(self, info, next_state):
        # to activate the caluclation of the custom reward change this to True
        use_custom_reward = False

        if use_custom_reward:
            # Implement your reward logic here
            # next_state has the next state object of type HacktrickState
            # to get current state use self.base_env.state
            pass
    ...
    def step(self, action_dict):
        ...
        """
        if self.mode == 'single':
            action = [action_dict[self.curr_agents[0]]]
            assert all(self.action_space.contains(a) for a in action), "%r (%s) invalid"%(action, type(action))
            joint_action = [Action.INDEX_TO_ACTION[action[0]], Action.STAY]
            ...
            raise 'mode has to be either single or collaborative'

        next_state, sparse_reward, done, info = self.base_env.step(joint_action, display_phi=self.use_phi)
        dense_reward = self.get_dense_reward(info, next_state)

        ob_p0, ob_p1 = self._get_obs(next_state)

        """
        shaped_reward is the total reward calculated from your score (sparse_reward) plus the dense_reward.
        dense_reward is weighted by an annealing factor that decreases over time to let your agent learn
        from its raw score only as time progresses.
        You can use the provided rewared function or implement your own in the get_dense_reward function above.
        If you choose to do so you can remove or change the annealing factor by changing self.reward_shaping_factor
        """
...
class TrainingCallbacks(DefaultCallbacks):
    ...
    def on_episode_end(self, worker, base_env, policies, episode, **kwargs):
        ...
        """
        # Get rllib.HacktrickMultiAgentEnv refernce from rllib wraper
        ...
        # Parse info dicts generated by HacktrickEnv
...
def evaluate(eval_params, mdp_params, outer_shape, agent_0_policy, agent_1_policy, agent_0_featurize_fn=None, agent_1_featurize_fn=None, verbose=False):
    ...
    # Wrap rllib policies in hacktrick agents to be compatible with Evaluator code
    agent0 = RlLibAgent(agent_0_policy, agent_index=0, featurize_fn=agent_0_featurize_fn)
    if eval_params['mode'] == 'single':
        agent1 = StayAgent()
    elif eval_params['mode'] == 'collaborative':
...
def gen_trainer_from_params(params):
    ...
    register_env("hacktrick_multi_agent", params['ray_params']['env_creator'])
    ...
    env = HacktrickMultiAgent.from_config(environment_params)
    ...
    def gen_policy(policy_type="ppo"):
        ...
        assert policy_type == "ppo", 'ppo is the main policy supported, remove this assertion only if you know what you are doing'
    ...
    self_play = iterable_equal(multi_agent_params['bc_schedule'], HacktrickMultiAgent.self_play_bc_schedule)
    if not self_play:
    ...
    trainer = PPOTrainer(env="hacktrick_multi_agent", config={
